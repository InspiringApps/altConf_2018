...
import SceneKit
	...
	// TODO: do away with this nonsense and create own enum with caes such as .topMiddle, bottomMiddle, leftCentered, rightCentered
...
extension SCNNode {
	...
	public func animatePositionBy(_ distance: Float, alongAxis axis: SCNVector3, withDuration duration: TimeInterval, completion: (() -> Void)? = nil) {
	...
	public func animateToRotationRadians(_ angle: Float, withDuration duration: TimeInterval, completion: (() -> Void)? = nil) {
	...
	public func animateRotationByRadians(_ angle: Float, withDuration duration: TimeInterval, completion: (() -> Void)? = nil) {
		let animation = CABasicAnimation(keyPath: "eulerAngles.y")
		animation.fromValue = self.eulerAngles.y
	...
	public func showPivot(_ color: UIColor = .red) {
	...
	public func pivotAtCorner(_ corner: RectCorner, showPivotWithColor pivotColor: UIColor? = nil) {
		...
		if let geometry = self.geometry as? SCNText, let text = geometry.string as? String {
			...
			switch corner {
			case .allCorners:
				pivot = SCNMatrix4MakeTranslation(Float(textSize.width / 2), Float(textSize.height / 2), 0)
			case .topLeft:
				pivot = SCNMatrix4MakeTranslation(0, Float(textSize.height), 0)
			case .topRight:
				pivot = SCNMatrix4MakeTranslation(Float(textSize.width), Float(textSize.height), 0)
			case .bottomLeft:
				pivot = SCNMatrix4Identity
			case .bottomRight:
				pivot = SCNMatrix4MakeTranslation(Float(textSize.width), 0, 0)
			default:
				pivot = SCNMatrix4Identity
			}
		}
		...
		if let size = sizeOfBoxOrPlane(self.geometry) {
			...
			case .allCorners:
				pivot = SCNMatrix4Identity
			case .topLeft:
				pivot = SCNMatrix4MakeTranslation(Float(-size.width / 2), Float(size.height / 2), 0)
			case .topRight:
				pivot = SCNMatrix4MakeTranslation(Float(size.width / 2), Float(size.height / 2), 0)
			case .bottomLeft:
				pivot = SCNMatrix4MakeTranslation(Float(-size.width / 2), Float(-size.height / 2), 0)
	...
	public func alignToPlaceholder(_ placeholderNode: SCNNode, atCorner corner: RectCorner, hoverDistance: Float = 0, showPivotWithColor pivotColor: UIColor? = nil) {
		...
		switch corner {
		case .topLeft:
			self.position.x -= Float(size.width / 2)
			self.position.y += Float(size.height / 2)
		case .topRight:
			self.position.x += Float(size.width / 2)
			self.position.y += Float(size.height / 2)
		case .bottomLeft:
		...
		self.position.z += Float(size.depth) + hoverDistance
	}
